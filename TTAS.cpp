...
class TTASLock {
    ...
    public:
        ...
        void lock() {
            while (true) {
                while(state.load()); //Spin
                if (!state.exchange(true)) return;
            }
        }
        void unlock() {
            state.store(false, memory_order_seq_cst);
...
void threadWork(int id, int numOps, double lamda1, double lamda2, double pl, double pi, TTASLock& lckObj);
...
void threadWork(int id, int numOps, double lamda1, double lamda2, double pl, double pi, TTASLock& lckObj) {
    ...
    string logFileName = "Thread_LogFile(TTAS)_" + to_string(id+1) + ".log";
